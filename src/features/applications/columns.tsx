...
import type { useEnumLabel } from '@/i18n/enum-labels';
  ...
  return [
    {
      ...
      cell: ({ row }) => (
        <Checkbox
          ...
          aria-label={t('applications.selectRow', { position: row.original.position })}
        />
      ),
    },
    {
      accessorKey: 'companyName',
      header: sortableHeader(t('common.company')),
      cell: ({ row }) => (
        <button
          className="block max-w-48 font-medium hover:underline text-left"
    ...
    },
    {
      ...
      header: sortableHeader(t('common.position')),
      cell: ({ row }) => (
        <button className="block max-w-48 text-left hover:underline" onClick={() => opts.onOpen(row.original.id)}>
    ...
    {
      accessorKey: 'status',
      header: t('applications.statusLabel'),
      cell: ({ row }) => (
        <div className="flex items-center gap-1.5">
          <StatusBadge status={row.original.status} />
          {isFollowUpNeeded(row.original) && (
            <AlertCircle className="size-3.5 text-amber-500" aria-label={t('applications.followUpNeeded')} />
          )}
        </div>
      ),
    ...
    },
    {
      ...
      header: t('applications.priorityLabel'),
      cell: ({ row }) => <PriorityBadge priority={row.original.priority} />,
      filterFn: (row, id, value: string[]) => !value?.length || value.includes(row.getValue(id)),
    },
    {
      accessorKey: 'source',
      header: t('applications.sourceLabel'),
      cell: ({ row }) => enumLabel('source', row.original.source),
      filterFn: (row, id, value: string[]) => !value?.length || value.includes(row.getValue(id)),
    },
    {
      id: 'salary',
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => opts.onOpen(row.original.id)}>{t('common.open')}</DropdownMenuItem>
